...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
先停止创建新任务，但不要直接杀死正在写入结果的任务；GB 千兆字节（Gigabyte）是磁盘容量单位；磁盘只剩 8 GB，继续增加任务可能耗尽空间并破坏尚未写完的文件
...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
$C_{pk}$ 是过程能力指数（Process Capability Index），用于比较生产波动和产品允许范围；数值从 $1.45$ 降到 $1.08$，说明生产结果比以前更容易靠近或越过允许边界，因此不合格品风险正在增加
...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
DSCR 偿债能力覆盖倍数（Debt Service Coverage Ratio）比较企业可用于还款的资金和同期需要偿还的本息；预测值为 $1.18$，表示预测资金只比预测还款金额高 $18\%$，缓冲空间较小
...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
单次结果为 $58$ 不能独立证明已经发生肾衰竭；小于 $60$ 可能提示肾脏过滤能力下降，而 $15$ 或更低通常才称为肾衰竭，所以这次结果需要复查，但不应直接把自己判断为肾衰竭
...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
订单积压正在扩大，因为每天新增 $62$ 笔订单，但每天只能完成 $55$ 笔；每天净增加 $7$ 笔，现有 $180$ 笔积压不会自行消失
...
过去十四天中，只有 $22\%$ 的出院患者在中午前离院；大量床位到傍晚才空出，上午到达的住院患者只能继续等待
...
基准预测的内部回报率为 $10.8\%$，公司的最低要求为 $9\%$

两者只相差 $1.8$ 个百分点，所以安全空间并不大
