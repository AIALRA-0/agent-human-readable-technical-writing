...
同时登记读者、文体、媒介、内容组件和用户配置；任务契约还要登记独立或连续阅读范围、已解释术语、原始组件顺序、排比组、代码覆盖方式、多轮替换关系、实际渲染证据、证据边界和横向布局例外；结构以 `contracts/task-contract.schema.json` 为准
...
- 代码：读取 `profiles/components/code.yaml`；保留原始代码，并选择合法代码注释或逐行独立解释，每个有效语句都要有可定位的说明；同时解释工具、动作、原因、语法、输入、结果、失败表现、副作用、查询方法和证据边界
