...
排比组由 Agent 按语义登记，表格的表头、行、单元格和顺序由 TABLE 组件覆盖，不作为散文排比组项目；程序检查登记项目是否逐项使用对应层级的标准 Markdown 列表标记并保持正确缩进；单纯换行、空格缩进或连续散文不能冒充 `indented_list`；不同列表类型的独立排比组之间允许一个块级空行，同一列表或嵌套列表内部的空行仍然失败；正则表达式只能提出候选，不能单独决定自然语言是否构成排比
...
注释代码以每个代码块的最长可注释代码显示宽度为基准；目标列必须等于最长宽度加二，每个可注释 `CODE` 单元都要在该列保持同行；局部修复可以调整生成的同行注释和对齐空格，但必须保持可执行代码、字面值、调用关系和控制条件不变；针对可配置或可迭代输入，次数、上限和失败边界必须绑定实际默认值或输入长度，有 n 个输入值时必须区分最多调用 n 次与最多重试 n-1 次，并单独保留空输入的零调用边界；正文复述源代码字面量时将实际值或表达式放入行内代码；调用次数、重试次数和空输入边界不得用中文分号压在同一列表项中；代码语言和标识符在正文中使用行内代码，代码语言名称不自动成为专业术语；用户参考中的受保护标识必须逐字保留；逐行解释路径不得登记伪造的注释对齐证据
