...
- 代码：读取 `profiles/components/code.yaml`；保留原始代码，并选择合法代码注释或逐行独立解释，每个有效语句都要有可定位的说明；使用注释时始终在每个代码块内按照最长可注释代码行同行对齐，允许对齐增加行宽；同时解释工具、动作、原因、语法、输入、结果、失败表现、副作用、查询方法和证据边界
