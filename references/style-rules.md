...
根据本技能的可读性取舍，固定 25字上限会拆碎术语全称、数值来源和必要条件，所以不用字数单独决定是否拆句
...
`根据运维团队最近 10次同规模迁移的经验，迁移时间通常为 25到30分钟；本次预留 30分钟属于经验估计，网络负载增加时仍可能超时`

新数值首次出现时还要说明取得方法，来源名称本身不能代替统计或计算过程

关系性数值表示一个数值由其他数值展开、汇总、去重或换算得到

错误

`74个多通道包装层按照各自通道数展开为 179个独立单端口存储体`

正确

`根据结构清单统计，74个包装层由 19个单通道、20个双通道、20个三通道和 15个四通道包装层组成`
...
去重计数也需要展示统计口径，例如 `45=\operatorname{count}(\operatorname{distinct}(shape))`，并说明 `shape` 字段来自哪份记录、哪些记录被纳入和排除
...
- 表题放在表格下方，编号章节中的写法为“表 1.1 候选结果”
- 图题放在图片或流程图下方，编号章节中的写法为“图 1.1 验证流程”
- 补充说明放在图表下方，使用“注：”

用户明确指定 IEEE 或目标出版规范时采用对应位置；IEEE 表题放在表格上方，图题放在图形下方
...
在一个单元格内写 `按时足量交付率（On Time In Full，OTIF）`；随后写“这个指标统计订单是否按时并且数量完整地交付；数值下降说明未满足交付承诺的订单增多”

### 15.2. 普通英文名词

普通英文名词已有自然、稳定的中文名称时，使用 `中文全称（English Name）`
...
产品、品牌、软件、型号、计量单位或其他名称没有自然通行的中文译名时，保留官方原生名称，并紧跟自然中文解释
...
生硬音译会丢失原始名称的可检索性，还可能产生错误名称，因此准确性和行业惯例优先于逐词翻译

### 15.3. 中英混合名称

不要把缩写和英文限定词整串复制到中文前

普通英文词已有自然中文表达时，不在正文括号外单独保留小写英文词

官方原生名称、规范单位名、程序标识和没有自然通行中文译名的名称不受这一限制，但首次出现后必须紧跟自然中文解释
...
它们使用的是已有自然中文名称的普通英文词，读者仍需要先跨过英文词；正确做法是先写中文名称，再保留原名并补充自然解释
...
已有正式英文全称的缩写使用“中文全称（English Full Name，缩写）”的连续顺序

英文全称优先采用官方写法

没有明确官方写法时，多词技术名称默认使用主要单词首字母大写

没有正式展开形式的格式名、产品名和品牌名不得编造全称；计量单位、数学符号、程序标识和有固定小写惯例的名称保留原样
...
`根据库存系统记录，仓库现有 426个传感器`

错误

`根据库存系统记录，仓库现有四百二十六个传感器`

“第一步”等操作顺序、固定短语、专有名称和逐字证据保持原样，不能为了统一外观机械替换
...
普通正文、列表项目和标题的行尾不使用中文分号或中文句号；Markdown 引用块、代码块、行内代码和明确标记的逐字日志保持原文
...
标题可以自然使用“和、与、及、顿号”；只要这些对象共同组成一个清楚主题，检查器不得仅凭连接词判定失败

斜线和连接符容易隐藏对象关系；使用它们时改成能够说明真实关系的标题
...
表题默认放在表格下方，编号章节中的写法为“表 1.1 标题”；用户明确指定 IEEE 或目标出版规范时采用对应位置
...
同行注释说明命令是什么、为什么执行和会产生什么可观察结果，不重复命令字面内容
...
独立注释说明这一段是什么、为什么存在和会产生什么结果，不要求在括号或纯结构行后机械重复
...
- 根据本技能设定，能在 15个字以内准确解释时仍然保留术语，例如在“加严抽检”后说明“提高抽样比例并扩大检查范围”
- 普通中文正文不得存在中文句号字符 U+3002；逐字引文、日志和代码保持原文
...
- 一句话不得被多个长括号切碎
- 一个标题需要准确概括同一章节内容；自然并列对象可以共同组成一个主题
- 非问答内容的标题必须使用陈述式主题
- 多章节内容必须使用从 `1.` 开始的十进制层级编号，三级和四级标题分别使用 `1.1.` 和 `1.1.1.`
...
- 表格和图形必须分别计数；默认题注位于对象下方，显式出版规范采用对应位置
- 引用必须使用顺序编码格式
...
- 冒号后的并列项目必须增加一级缩进，列表项目内部再次出现并列内容时继续增加一级缩进
- 实质流程达到三个以上有效节点、包含分支、重试、多方参与或返回前一步时必须使用 Mermaid
- 单结果条件必须与结果保持在同一项目中
- 多结果条件必须把后续行动或后果换行缩进
