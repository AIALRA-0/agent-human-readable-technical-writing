...
案例来源标识为：`new-arabic-digits-native-unit-name`

#### 2.1.1. 问题

> 仓库照度记录写成“四百二十六拉克丝”，帮我改成普通人看得懂、还能查原始单位的说法
...
案例来源标识为：`new-relational-number-derivation`

#### 2.2.1. 问题

> 把这些统计写成能够复算的说明：74个多通道包装层展开为179个独立存储体；193个语义存储体保存8,605,650位，约1.026 MiB
...
> 写一段项目周报，带结果表；使用默认题注位置，章节标题不要写成问题
...
> 按照 IEEE 出版格式整理下面的测试结果，表题使用 IEEE 位置，章节标题使用陈述句
...
案例来源标识为：`new-precise-maintenance-verbs`

#### 2.44.1. 问题

> 把“先进行日志检查，再完成对故障原因的分析，最后执行维修单填写”改成人话
...
案例来源标识为：`new-single-scope-release-note`

#### 2.45.1. 问题

> FPGA 软件检查过了，板卡没测，客户也没验收，写一段发布范围说明，别连续写三遍“不代表”
...
案例来源标识为：`new-natural-engineering-retrospective`

#### 2.46.1. 问题

> 把这段机器味很重的内容改成工程师复盘：已经完成了对 API 接口故障的分析，进行了日志检查和请求验证，执行了限流配置调整，这不代表故障彻底解决，也不代表所有客户都恢复
...
案例来源标识为：`new-owner-led-action-list`

#### 2.47.1. 问题

> 质量负责人要复核记录、批准放行、保存证据，现场操作员要隔离货物，怎么写才能既不重复啰嗦又不丢责任人
...
案例来源标识为：`new-large-task-negation-drift-gate`

#### 2.49.1. 问题

> 把跨部门迁移复盘改成长报告，原稿每章都在用“不是……而是……”和“不在于……而在于……”，请彻底改成直接因果表达：已知事实包括审批记录缺负责人、回滚演练通过、切换后的订单核对缺少两小时记录和客户验收尚未开展
...
案例来源标识为：`new-editorial-meta-narrative-rewrite`

#### 2.50.1. 问题

> 把这句话改成正式报告正文：文档状态：第一部分已经形成真实截图、命令输出和智能体连接记录，后续章节将在同一时间线上继续补充
...
案例来源标识为：`new-paragraph-scope-tool-handover`

#### 2.51.1. 问题

> 把一份交接说明改自然：原稿每句话都重复“工程团队”，每一章又重新解释 Vivado、ModelSim 和 SynthPilot，步骤也都挤在普通段落里
