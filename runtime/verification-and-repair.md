...
注释代码以每个代码块的最长可注释代码显示宽度为基准；目标列必须等于最长宽度加二，每个可注释 `CODE` 单元都要在该列保持同行；局部修复可以调整生成的同行注释和对齐空格，但必须保持可执行代码、字面值、调用关系和控制条件不变；针对可配置或可迭代输入，次数、上限和失败边界必须绑定实际默认值或输入长度；逐行解释路径不得登记伪造的注释对齐证据
