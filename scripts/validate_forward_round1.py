...
def main() -> int:
    ...
    report = {
        ...
        "current_candidates": {
            "pending": 0,
            "case_ids": [],
            "all_explicitly_accepted": True,
        },
        "perfect_round_streak": {"current": 0, "required": 2, "release_gate_met": False},
        "next_round_allowed": True,
        "reason": "首次答案、请求摘要和已登记机械错误保持一致；第一轮成绩仍为 8/20，五个修订候选已经全部获得用户明确接受" if not evidence_errors else "前向证据与实际检查结果不一致",
        "impact": "第一轮成绩固定为 40%，修订版不替换该成绩；当前允许生成第二轮全新未见案例" if not evidence_errors else "当前记录不能证明第一轮的真实结果",
        "next": "生成并冻结第二轮 20 个全新未见首稿" if not evidence_errors else "修复证据记录或生成物不一致问题",
    }
