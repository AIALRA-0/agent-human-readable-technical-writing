...
排比组由 Agent 按语义登记，表格的表头、行、单元格和顺序由 TABLE 组件覆盖，不作为散文排比组项目；程序检查登记项目是否逐项使用对应层级的标准 Markdown 列表标记并保持正确缩进；单纯换行、空格缩进或连续散文不能冒充 `indented_list`；正则表达式只能提出候选，不能单独决定自然语言是否构成排比
