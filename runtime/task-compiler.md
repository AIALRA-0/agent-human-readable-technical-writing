...
独立阅读单元中确属专业名词的内容需要登记官方中文、官方英文、缩写、中文定义、名称含义、名称依据、当前作用、可观察表现和实际影响；生成时必须按注册表的 `required_meanings` 在首次语义位置落实这些含义，其中 `name_origin` 使用 `name_rationale` 的实际含义，不能只写一般定义或依靠后置术语表；`core_terms`、主题关键词和普通操作短语不自动触发该合同；没有正式身份的普通任务标签用自然中文解释，不创造英文形式；确属专业名词但官方写法无法核对时，编译结果进入 `BLOCKED`

多轮任务通过 `conversation` 登记已经失效的要求；追加要求替换旧要求后默认只保留当前有效内容，用户要求变更历史、撤销说明、审计或证据核对时才把保留开关设为 `true`

只有任务媒介或合同明确选择 `github_markdown` 时，GitHub 图片、表格和 Mermaid 任务才需要登记 1280 像素与 390 像素、亮色与暗色四种实际渲染证据；源码中的居中标记不能代替页面结果；其他媒介不得伪造 GitHub 渲染证据
