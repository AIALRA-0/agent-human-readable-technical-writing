...
{seed}
"""
    reference_tokens = required_reference_tokens(request)
    reference_instruction = (
        "No stable reference identifier was supplied"
        if not reference_tokens
        else (
            "The following stable reference identifier(s) are protected answer content and must appear "
            "character-for-character in the answer, preferably as natural audit metadata; do not omit, "
            "rename, translate, or hide them: " + ", ".join(f"`{token}`" for token in reference_tokens)
        )
    )
    return f"""Use the installed $human-readable-technical-writing Skill for one isolated Chinese writing case.

For every independently presented `indented_list` group, each item must be a separate standard Markdown list item beginning with `- `, `* `, `+ `, or an ordered marker at the required depth; a bare newline or spaces without a marker is invalid. TABLE headers, rows, cells, and their order are component structure and must not be declared as prose `parallel_groups`; declare only independently authored prose outside the table. For TABLE output, explain the table's overall purpose and then cover every column by name, unit, allowed value or content range, and actual role; an aggregate mention of only some columns is incomplete. For code with configurable or iterable inputs, bind every count, upper bound, and failure boundary to the exact default value or input length; never turn a default bound into an unconditional function behavior. For an iterable with n supplied values, state at most n calls and, when n is greater than zero, at most n-1 retries; state the empty-input zero-call boundary separately. Put code languages, functions, variables, attributes, calls, exceptions, and literals in backticks in prose; a code language name is not automatically a professional term and does not need invented English expansion. {reference_instruction}. Before returning, audit every Chinese semicolon: keep it only for same-line compact items or a professional-term explanation, and split evidence impact, decisions, conditions, actions, metadata, and next-check directions when their semantic roles differ.
...
For code explanations, wrap every source-code value or expression repeated in prose in inline code, including numeric literals such as `1`, `2`, `3`, `0`, and symbolic bounds such as `n` and `n-1`; the code fence itself is already verbatim. Use separate Markdown list items or separate sentences for call count, retry count, and empty-input behavior, never one Chinese-semicolon list item. If the source is an iterable loop, state the exact relationship between supplied values, calls, retries, waits, and failure, including that an empty iterable performs zero calls
...
When different semantic parallel groups appear in one answer, keep each group in its own block with a natural introduction, suitable heading, or clear block boundary; do not flatten definitions, readings, causes, results, conditions, or actions into one same-level list. Before returning, compare each declared group with the actual list level and verify that adjacent groups are not silently merged.

Only treat a phrase or clause as a parallel-group item when it is independently presented, ordered, compared, or judged. A clause embedded inside another list item, such as a definition's example value, is not a separate group item; keep it in the containing item's declaration and do not force an answer rewrite just to satisfy stale metadata.
...
For every independently presented `indented_list` group, record the actual list-marker layout and preserve one exact single-line `item_texts` value for each item; a bare newline or spaces without a marker is not an indented list. TABLE headers, rows, cells, and their order are component structure and must not be declared as prose `parallel_groups`; remove any table-row group from the manifest without changing the table. For TABLE output, check that the frozen answer explains the overall purpose and every column's name, unit, allowed value or content range, and actual role outside the table. For code with configurable or iterable inputs, bind every count, upper bound, and failure boundary to the exact default value or input length; never leave a default bound as an unconditional statement when custom input can change it.
...
Only declare a group when its items are independently presented, ordered, compared, or judged. A phrase or clause embedded inside another list item, such as a definition's example value, is not a separate group; keep it in the containing item's declaration and do not force an answer rewrite just to satisfy stale metadata
...
def review_prompt(
    ...
    reference_tokens = list((evidence or {}).get("required_reference_tokens", []))
    reference_instruction = (
        "No protected reference identifier is present"
        if not reference_tokens
        else (
            "Check that every protected reference identifier appears character-for-character in "
            "CURRENT_ANSWER and is not renamed or omitted: " + ", ".join(reference_tokens)
        )
    )
    return f"""Use the installed $human-readable-technical-writing Skill as an independent semantic verifier.

TABLE headers, rows, cells, and their order are component structure, not prose `parallel_groups`; do not report a table-row layout as a list defect or require converting a table to prose. Inspect TABLE coverage separately, including the overall purpose and every column's name, unit, allowed value or content range, and actual role.

For code with configurable or iterable inputs, bind every count, upper bound, and failure boundary to the exact default value or input length; never turn a default bound into an unconditional function behavior. For an iterable with n supplied values, verify that the answer distinguishes at most n calls from at most n-1 retries when n is greater than zero, and separately covers the empty-input zero-call boundary. Verify that code languages, functions, variables, attributes, calls, exceptions, and literals are marked as inline code in prose; a code language name is not automatically a professional term and does not require invented English expansion.

Audit every Chinese semicolon in the current answer: it is valid only for same-line compact items or a professional-term explanation; evidence impact, decisions, conditions, actions, metadata, and next-check directions with different roles must be split into separate sentence or block units.

For code cases, treat missing inline-code markers on source-code literals as a real blocking finding, not an optional style preference. Treat a semicolon joining call-count, retry-count, or empty-input conditions as a real blocking finding and require separate list items or sentences. Verify protected reference identifiers character-for-character, including review numbers such as B29
...
`background_claims` is a top-level initial-output field and is not part of `CURRENT_MANIFEST` or closure-patch `updated_manifest`. When a registry-supported term or definition is already grounded by `SOURCE_AND_BACKGROUND_EVIDENCE`, do not require that background claim to be copied into `CURRENT_MANIFEST` as a separate patchable field. Report only an actually unsupported or missing background source, and treat any stale patchable manifest declaration as a local FAIL rather than a `REVIEW_REQUIRED` blocker.
...
Only treat a phrase or clause as a parallel-group item when it is independently presented, ordered, compared, or judged. A clause embedded inside another list item, such as a definition's example value, is not a separate group item; correct the manifest locally instead of requiring an answer rewrite solely for stale metadata. For an independently presented `indented_list` group, require a standard Markdown list marker on every item at the required depth; a newline without a marker does not satisfy the layout rule.
...
Recheck that TABLE rows remain component structure rather than prose `parallel_groups`, that every table column is covered outside the table, and that code counts and failure boundaries remain bound to default values or input lengths. For an iterable with n supplied values, verify at most n calls, at most n-1 retries when n is greater than zero, and the separate empty-input zero-call boundary; verify inline code markup for code languages and identifiers plus every protected reference identifier.
...
3. every parallel group, nested list, semicolon, natural list introduction, and boundary between different semantic groups
...
When a supplied finding concerns professional first use, consult that term's registry `required_meanings` and put every missing meaning into the same earliest-occurrence sentence; when `name_origin` is missing, express the registry `name_rationale` meaning explicitly in the replacement, not merely a general definition.
...
For code boundary repairs, keep source-code values and expressions in inline code in prose, use separate list items or sentences for calls, retries, and empty input, and preserve the exact supplied reference token named by `PROTECTED_TOKEN_PRESENCE`; do not combine those changes into overlapping patches
Do not submit two patches that touch the same line or overlapping text; combine all changes to one sentence or line into one exact patch against the unchanged CURRENT_ANSWER. If a `PROTECTED_TOKEN_PRESENCE` finding names a reference identifier, copy that identifier character-for-character into the repaired answer. For code, preserve executable code and explain an iterable with n values as at most n calls and, when n is greater than zero, at most n-1 retries, with a separate empty-input zero-call boundary; use inline code for code languages and identifiers in prose.
{rejection}
