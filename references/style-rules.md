...
个人文档、普通说明、项目主页、笔记和一般技术报告默认使用下面的题注位置：

- 表题放在表格下方，编号章节中的写法为“表 1.1 候选结果”
- 图题放在图片或流程图下方，编号章节中的写法为“图 1.1 验证流程”
- 补充说明放在图表下方，使用“注：”

两个题注都放在下方能够保持视觉位置统一，也符合个人文档的默认阅读偏好

用户明确要求论文、期刊、会议、标准、审计出版物或指定出版模板时，把表题放在表格上方，把图题放在图形下方

用户没有明确指定文档类型时，根据交付目的判断；存在疑问时优先采用表题与图题都在下方的个人文档方案

正式出版物另有模板时遵循出版模板

同一文档内的同类对象需要保持相同位置，除非指定模板明确要求例外
...
过程能力指数的规范数学记号是 `$C_{pk}$`；其中下标 `$pk$` 表示同时考虑生产结果是否偏离目标中心以及是否接近上下规格边界；`$C_{pk}$` 不是需要全部大写的普通英文缩写
...
个人文档默认把表题放在表格下方，编号章节中的写法为“表 1.1 标题”

明确要求正式出版格式时，才把表题放在表格上方
...
- 每个解释单位中的首次内部名称必须用自然短句说明它代表什么、为什么重要以及会造成什么
- 一句话不得被多个长括号切碎
- 一个标题只能包含一个主题
- 非问答内容的标题必须使用陈述式主题
- 多章节内容必须使用从 `1` 开始的十进制层级编号
- 操作步骤必须使用中文顺序，并在相邻步骤之间保留空行
