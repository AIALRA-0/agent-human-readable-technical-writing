...
def build_cases() -> list[dict[str, object]]:
    ...
    cases += rule_cases("status_audit", [
        ("按项目变化七张卡写状态：新增 `verify.py`；为避免漏检；验证 32/32；尚未推送；证据只有本地测试输出", evaluator("status_aemp", ["新增|改了什么", "避免.{0,12}漏检|原因", "32/32", "未推送", "本地测试|证据"], [])),
        ("解释审计结论并绑定主张与证据：Git status 显示 2 个未跟踪文件；没有远端证据；不要推断已推送", evaluator("status_aemp", ["2", "未跟踪", "(?:无|没有|缺失).{0,20}远端|远端.{0,20}(?:无|没有|缺失)", "未知|无法确认|不能.*写成|不能.*证明"], [])),
        ("写事故状态：测试进程退出码 1；日志最后一行是 schema mismatch；根因尚未证明", evaluator("status_aemp", ["退出码.{0,8}1", "schema mismatch", "根因.{0,16}(?:未知|未证明|仍在调查|尚未确认)|(?:证据不足|无法).{0,16}(?:证明|确认).{0,8}根因"], ["根因是"])),
        ("写删除测试结论：删掉来源句后，结论失去唯一证据，因此不能保留；说明为何重要", evaluator("status_aemp", ["删除", "证据", "不能保留|失去"], [])),
        ("用三层展示写状态：先给结论，再给最小上下文，最后给证据；事实是候选通过、用户尚未接受", evaluator("status_aemp", ["候选", "通过", "用户", "尚未.{0,8}用户接受|用户.{0,12}接受.{0,12}不存在"], [])),
