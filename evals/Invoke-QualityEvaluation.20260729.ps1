...
$currentGeneration = '2026-08-05-arabic-digits-native-names'
$paragraphScopeGeneration = '2026-08-03-paragraph-scope-step-warning'
...
- 如果距离到期超过 30天，按照合同规定的送达方式发送不续期通知
- 如果距离到期不足 30天，自动续期可能已经无法单方面阻止，需要与对方协商
...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
根据用户本次提供的打印机成本数据，3年内预计打印 12万页，耗材费用会远高于 2000元的购买差价，所以不能只比较机器价格
...
根据表 1 的成本数据，乙方案虽然购买时多花 2000元，但 3年合计少花 9000元，因此预计打印量能够实现时应选乙方案
...
- 19:05 订单资料复制完成
- 19:18 开始逐日比较订单金额
- 21:40 发现 7月12日金额相差 320元
- 22:10 确认差异来自一笔已经取消但未同步状态的订单
- 22:35 修正转换规则并重新校验 7月12日
...
根据迁移切换计划，RTO 恢复时间目标（Recovery Time Objective）表示发生故障后业务最多允许中断多久；本次要求为 30分钟

这两个要求决定切换必须具备下面三项准备：

- 旧系统保持可用
- 切换前记录最后一笔订单编号
- 新系统异常时能够在 30分钟内恢复旧系统写入
...
- 过去 14天平均每天有 68名急诊患者需要住院
- 同期平均每天只有五十九张床位能够腾出
- 每天平均新增九名等待床位的患者
- 等待超过 12小时的患者从每天 11人增加到每天 27人
- 周末能够腾出的床位比工作日少 $31\%$
...
根据医院出院记录，过去 14天中只有 $22\%$ 的出院患者在中午前离院；大量床位到傍晚才空出，上午到达的住院患者只能继续等待
...
根据项目模型，售电价格在 25年内基本稳定，但合同只锁定前 8年；第 9年后的收入存在明显不确定性
...
- 如果三项分别发生，项目净现值仍为正
- 如果三项同时发生，净现值降为 -900万元，内部回报率降到 $7.6\%$
...
根据研究摘要，RCT 随机对照试验（Randomized Controlled Trial）通过随机分组减少两组参与者原有差异；本研究把 120名参与者随机分成训练组和对照组
...
根据用户提供的笔记，旧版本恢复演练从发现异常到恢复服务共用 18分钟
...
根据用户提供的笔记和现有日志，API 应用程序接口（Application Programming Interface）负责在不同系统之间传递请求和结果；当前接口大约每 1000次请求出现 3次超时
...
'@
    ...
    [pscustomobject]@{
        ...
        Structure = 'declarative-headings-table-title-below'
        Prompt = '写一段项目周报，带结果表；使用默认题注位置，章节标题不要写成问题'
        Response = @'
...
'@
    ...
    [pscustomobject]@{
        ...
        CaptionStyle = 'IEEE'
        Prompt = '按照 IEEE 出版格式整理下面的测试结果，表题使用 IEEE 位置，章节标题使用陈述句'
        Response = @'
...
根据项目看板记录的 41项已关闭任务和 50项总任务重新计算：
...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
根据用户本次提供的数据，主传感器连续 20分钟记录为 $7.8\,^\circ\mathrm{C}$
...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
根据用户本次提供的试验记录，甲方法和乙方法各执行 12次，因此两组错误次数可以在相同试验次数下比较
...
根据用户提供的同一组试验记录，两组试验各只有 12次，而且原始材料没有提供输入难度和错误类型
...
'@
    ...
    [pscustomobject]@{
        ...
        Response = @'
根据用户本次提供的报告，RPO 恢复点目标（Recovery Point Objective）为 15分钟；系统发生故障后，最近 15分钟内尚未进入备份的数据可能需要重新录入

RTO 恢复时间目标（Recovery Time Objective）为 2小时；故障发生后，技术团队计划在 2小时内恢复服务

业务负责人需要分别确认 15分钟的数据损失是否可以接受，以及 2小时的停机是否会影响关键业务
'@
    },
    [pscustomobject]@{
...
$$
0.01\,\mathrm{g} = 10.02\,\mathrm{g} - 10.01\,\mathrm{g}
$$

$$
0.39\,\mathrm{g} = 10.40\,\mathrm{g} - 10.01\,\mathrm{g}
$$
...
'@
    ...
    [pscustomobject]@{
        ...
        IntroducedIn = $paragraphScopeGeneration
        Length = 'medium'
        Difficulty = 'hard'
...
'@
    ...
    [pscustomobject]@{
        Id = 'new-arabic-digits-native-unit-name'
        IntroducedIn = $currentGeneration
        Length = 'short'
        Difficulty = 'medium'
        Direction = 'numeric-and-native-name-rewrite'
        Tone = 'plain-technical'
        Audience = 'warehouse-manager'
        TaskType = 'measurement-rewrite'
        InputShape = 'hard-transliteration-and-written-number'
        Structure = 'source-result-boundary-action'
        RequiredTerms = @('lux')
        Prompt = '仓库照度记录写成“四百二十六拉克丝”，帮我改成普通人看得懂、还能查原始单位的说法'
        Response = @'
根据用户提供的仓库照度记录，工作台照度为 426 `lux`

`lux` 是衡量照度的国际通用单位名称，数值越高，单位面积接收到的光越多

原记录没有提供仪器型号、校准时间和测量位置，因此 426 `lux` 只能代表这次记录中的测量结果

仓库负责人需要核对测量记录和工作区域的照明要求，再决定是否调整照明
'@
    },
    [pscustomobject]@{
        Id = 'new-relational-number-derivation'
        IntroducedIn = $currentGeneration
        Length = 'long'
        Difficulty = 'hard'
        Direction = 'relational-number-derivation'
        Tone = 'audit-ready'
        Audience = 'fpga-engineer'
        TaskType = 'numeric-derivation'
        InputShape = 'aggregate-counts-and-unit-conversion'
        Structure = 'source-components-formulas-conclusion'
        RequiredTerms = @('MiB')
        Prompt = '把这些统计写成能够复算的说明：74个多通道包装层展开为179个独立存储体；193个语义存储体保存8,605,650位，约1.026 MiB'
        Response = @'
根据用户提供的结构清单，74个多通道包装层由下面4组构成：
...
每个通道展开为1个独立单端口存储体，展开后的数量按照下面公式计算：

$$
179 = 19 \times 1 + 20 \times 2 + 20 \times 3 + 15 \times 4
$$

根据用户提供的语义存储清单，193个语义存储体分为下面3组：

- 100个甲类存储体，共保存 4,000,000位
- 80个乙类存储体，共保存 4,000,000位
- 13个丙类存储体，共保存 605,650位

存储体数量和有效数据位数分别按照下面的公式核对：
...
$$
8{,}605{,}650\ \mathrm{bit} = 4{,}000{,}000 + 4{,}000{,}000 + 605{,}650
$$

MiB 二进制兆字节（Mebibyte）使用 $1024^2$ 个字节作为1个容量单位，每8位等于1字节

位数先除以8得到字节数，再除以 $1024^2$ 得到二进制兆字节：

$$
\frac{8{,}605{,}650}{8 \times 1024^2}
= 1.025873\ldots\ \mathrm{MiB}
\approx 1.026\ \mathrm{MiB}
$$

计算结果按3位小数显示为 1.026 MiB，原始位数仍保留为 8,605,650位，读者可以使用同一公式重新核对
'@
    }
)
...
$results = foreach ($case in $cases) {
    ...
    $lintArguments = @{ Text = $case.Response; CaptionStyle = $captionStyle }
