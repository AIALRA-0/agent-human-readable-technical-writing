...
<!-- Repository quality-check markers 读取自仓库测试脚本和完整案例文档: rule_checks-224- full_QA_cases-51- -->
...
Mermaid 文本流程图是一种使用文字定义节点和连接关系的图，本仓库用它展示处理顺序；读者沿箭头即可核对每一步怎样到达下一步
...
表题、图题和流程图题默认放在对象下方；用户明确指定 IEEE 或其他出版规范时，按照目标规范调整位置 [1]
...
例如多通道对象从 74个展开为 179个时，正文需要列出下面的组成：
...
决策、状态、审计和项目变化说明还会建立内容充分性判断，明确首屏必须出现什么、哪些证据支持哪些结论，以及哪些细节应该下钻查看

删除一句话、卡片或图例后，如果读者的理解、判断、行动、证据边界和下一步都不改变，这项内容就会被删除、折叠或移入高级层
...
| AEMP 内容充分性规则 | 需要设计首屏、解释变化、支持决策、展示状态或审计证据时 | 确定读者任务、必需内容、证据标签、下钻层级和删除测试 |
...
当前版本把确定性检查、事实保真评估、自然度评估和用户偏好分开；原因是单一检查器无法同时证明格式正确、关系不变和中文自然
...
- 来源文本中的机器字段、网址、路径、版本、数值或重复次数在候选文本中丢失

下面这些问题依赖全文语境，正则检查器只提供定位提醒：

- 术语是否已经获得当前读者所需的解释
- 并列内容是否真的需要拆成列表
- 主体、代词、版本、来源和因果关系是否清楚
- 双重否定、名词化、长句、空洞引导和编辑过程说明是否影响当前场景
- 内部术语是否泄漏到普通聊天、README 或 UI

这些提醒必须进入后续语义和自然度评估；它们不由正则直接宣布通过或失败，因为机械清零会再次把正常中文压成模板文体

第二类检查使用 20个错误版本和 20个正确版本，覆盖事实关系、术语解释、逻辑连续性、重复结论、代码说明和流程图；事实保真失败会直接阻止候选版本，自然度结果不能覆盖事实错误

第三类检查使用完整问题和完整回答，验证不同篇幅和不同场景，避免项目只对少数样例表现良好

第四类检查使用 72组最小对照和 33条经过脱敏的用户偏好；模型评分帮助定位问题，用户确认仍是最终风格标准

根据当前仓库中的测试脚本和完整问答案例自动统计，公开测试包含下面这些差异：

- 224组确定性规则正反测试
- 51组完整问答测试
- 40个语义对抗案例，其中错误版本和正确版本各 20个
- 72组自然中文最小对照
- 33条脱敏用户偏好
- 24组原始术语保留问答测试
- 5种回答长度
...
```powershell
pwsh -NoProfile -File ".\scripts\Test-HumanReadableChinese.Tests.ps1" # 执行 224组确定性规则正反测试
pwsh -NoProfile -File ".\evals\Invoke-QualityEvaluation.20260729.ps1" # 执行 51组完整问答质量测试
pwsh -NoProfile -File ".\evals\Invoke-NaturalChineseEvaluation.ps1" -Mode ValidateCases # 验证 40个语义正反案例的数量、配对和字段
pwsh -NoProfile -File ".\evals\Invoke-NaturalChineseEvaluation.ps1" -Mode RunAdversarial -AuthPath "<Codex 登录文件路径>" -OutputPath "<仓库外私有报告路径>" # 在两个隔离的新任务中独立判断事实保真和自然度
pwsh -NoProfile -File ".\evals\Measure-NaturalChineseLive.ps1" -Mode RunAll -SkillRoot "." -AuthPath "<Codex 登录文件路径>" -OutputDirectory "<仓库外私有评测目录>" # 执行 72 次触发、48 个现场生成和 24 组盲评准备
python -m unittest discover -s tests -p "test_*.py" -v # 验证公开语料扫描范围、相对路径、脱敏和代码围栏识别
...
确定性检查只能发现能够逐字确认的问题；独立语义评估能够发现关系漂移，但模型判断仍可能出错，因此正式版本需要用户盲评校准
...
| `THIRD_PARTY_NOTICES.md` | 记录自然化方法参考、作者、来源和许可证 |
| `references/natural-chinese.md` | 8种读者场景、上下文术语和翻译腔诊断 |
...
| `scripts/audit_aialra_corpus.py` | 只扫描公开仓库和明确审核样本，并输出脱敏后的相对路径报告 |
...
| `evals/Invoke-NaturalChineseEvaluation.ps1` | 在隔离任务中分别判断事实保真和自然度 |
| `evals/Measure-NaturalChineseLive.ps1` | 执行 72 次触发、48 个现场生成和 24 组 A/B 盲评准备 |
| `evals/activation-cases.jsonl` | 保存 24 个显式触发、隐式触发和边界请求 |
| `evals/fresh-generation-prompts.jsonl` | 保存 8 种场景各 6 个现场生成请求 |
| `evals/semantic-adversarial.jsonl` | 保存 20个错误版本和 20个正确版本 |
| `evals/minimal-pairs.jsonl` | 保存 72组上下文术语和自然表达对照 |
| `evals/user-preferences.jsonl` | 保存 33条已经脱敏并由用户明确指出的偏好样本 |
...
公开语料扫描器只接受已确认的 GitHub 仓库或逐个传入的审核样本；报告使用仓库相对路径，并在写出片段前遮盖秘密和个人绝对路径
