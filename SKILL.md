...
两个以上承担相同作用的定义、步骤、比较对象、事实、原因或动作必须换行并缩进；是否出现冒号不影响判断，项目内部还有并列内容时继续增加一级缩进；压缩任务减少重复表达，同时保留主题边界和必要换行
...
- 代码：读取 `profiles/components/code.yaml`；保留原始代码，并选择合法代码注释或逐行独立解释，每个有效语句都要有可定位的说明；使用注释时在每个代码块内计算最长显示宽度 `L`，宽度为 `W` 的行补 `L-W+1` 个空格后写注释标记；两个以上可注释语句必须把代码块正文通过标准输入交给 `runtime/align_inline_comments.py` 机械对齐，再对最终正文运行同一脚本的 `--check`，只有返回 `PASS` 才能输出；允许对齐增加行宽；同时解释工具、动作、原因、语法、输入、结果、失败表现、副作用、查询方法和证据边界
- 来源呈现：短篇逐字文本使用引用块，代码与日志使用代码块，图片与表格保留原始对象；生成结论和普通强调不使用引用块
- 媒介布局：GitHub 图片和表格使用居中 HTML，Mermaid 保留原生代码块并使用居中 HTML 题注；桌面、移动端、亮色和暗色实际渲染都通过后才能声明居中完成
- 术语：读取 `registries/terms.yaml`；独立阅读单元中的专业名词首次出现时说明正式中文、官方英文、正式缩写、中文定义、名称含义、当前作用和实际影响；翻译任务默认保留专业名词的官方英文名称；未登记术语先核对官方来源，无法确认时进入人工复核
